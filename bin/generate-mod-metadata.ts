...
import type { IMod } from '../lib/modloader/ModLoader';
...
async function run(command: string, configPath: string): Promise<void> {
  // Create mod loader
  const config = <Record<string, unknown>>JSON.parse(fs.readFileSync(configPath, 'utf8'));
  const modLoader = new ModLoader({
    mods: <IMod[]>config.mods,
    path: join(process.cwd(), 'server'),
    loader: 'forge' in config ?
        { versionForge: <string>config.forge } :
        { versionNeoForge: <string>config.neoforge },
    versionMinecraft: <string>config.minecraft,
  });
